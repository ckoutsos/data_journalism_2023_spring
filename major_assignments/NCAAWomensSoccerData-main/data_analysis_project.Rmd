...
#mascot_matches <- mascot_list %>% 
  #inner_join(match_all, by = c("team"))

#match_all_test <- match_all %>%
  #group_by(opponent) %>%
  #summarise(count = n()) 


mas_outcomes <- match_all %>%
  select(outcome, team, opponent, opponent_id, team_id) %>%
  left_join(mascot_list, by = c("team"))

team_crosswalk_and_outcomes <- mas_outcomes %>%
  left_join(team_crosswalk, by = c("opponent_id", "team_id"))
