...
**Answer** rename data frame, turns number of month into abbreviation, group by the month and find the total number of calls in each month, arranged in order of most to least.
...
**Task** Run the following code. Explain what you see after running the code in the space below.
**Answer** I see bars! and charts! and bar charts! and numbers and titles of how many calls occur per month
...
**Answer** we add using the plus sign. so we added things to our chart. The chart is in increasing order with the lowest amount of calls to the left and the most amount of calls to the right.
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer** Flipped it to a horizontal bar chart with coord_flip (data is easier to see)
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer** made the chart more minimal (less annoying tick marks, white background, etc)
...
**Answer** ok so now it's blue?? and white and gray
```{r}
baltco_911_calls_by_month %>%
  ggplot() +
  geom_bar(aes(x=reorder(month,total_calls), weight=total_calls)) +
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer** wow big title/headline at the top and smaller axis labels on the side (month and count) and the source at the bottom. also headline describes what it DOES not what it IS
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer** i see a new dataframe with the date and the total calls on that date 
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer** it's a line chart with spikes (geom_line) aes x axis is date and y axis is number of calls on date (you can see the range on most days)
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer** terrible and very crowded (each week is shown) dataframe ggplot geome line and then scale x axis
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer** still no, slightly less bad though. angled x axis information 
```{r}
baltco_911_calls_by_date %>%
  ggplot() + 
  geom_line(aes(x=date, y=total_calls)) + 
...
"chart making is about creating a chart and making it progressively less terrible" - derek willis