...
**Answer** They do have the same structure, slightly different data in party registrant but the county column is the same.
...
(Stacking data on top of each other: has to be identically structured data, if the caps aren't identical, etc clean up/rename before if needed!)
...
**Answer** The amount of people who register for Democrats and Republicans increases in Frederick over time, and Democrats grow steadily to a higher number than Republicans. The amount of Libertarians dips in 2020 and then increases, and Green party decreases. Unaffiliated and other both increase. Overall, the total voters increase.
...
**Answer** Should have 24 rows. We do have 24!
...
1st thing we're doing: whenever we're joining, create a new dataframe (give it a new name)
2nd thing we're doing: it guessed to join by County, don't do this. specify the join column because that way you remember what you're doing
...
**Answer** A Geocode, race (different aspects of that), one race/two race population, and population of people 18 or older
...
By clause slightly different: Name is the county information by=c
...
**Answer** (We don't need totals we have them, we're just calculating percentages) Prince George's County is the highest percentage, and it also has the highest percentage of Democrats. Somerset County has 39% Black population adults and 39% Democrat, approximately 
...
**Answer** The highest percent is Howard, followed by Montgomery. Much smaller part of the population even with high democrat percentage. 
...
Sometimes joins look like they should work but don't. Often this is due to the two columns you're joining on having different data types: joining a <chr> column to a <dbl> column, for example. Let's walk through an example of that using some demographic data by zip code. Have to join same data type! Look at the rows and make sure they're the same!
...
**Answer** Changes the datatype to a character, so we could join it with a zipcode data that's a character. 
