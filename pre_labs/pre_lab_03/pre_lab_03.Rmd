...
**Task** Load some [Maryland state government payments data](https://opendata.maryland.gov/Budget/State-of-Maryland-Payments-Data-FY2008-to-FY2023/7syw-q4cy) by running the following code. We'll use the guess_max() function as an argument to use the first 10 rows to set the data type. What does the first line of the red Warning message that prints out when you load the data say? What happens when you follow its instructions? Answer below. **Answer** Warning: "call problems", then we made it problems(payments) (does this type of data match what I expected to see). So when I look at data it's expected to see a double and instead it's other things: XXX, V7P, etc. None of these values were in the first 10 rows. So basically it destroyed data when we loaded the CSV file.
...
**Task** Run the following codeblock to reload the data, using every row to set the data types. Does it show any parsing errors when you run? Answer below **Answer** Now, we have Vendor Zip in character column and not data column. 
...
**Task** Run the following codeblock to glimpse the data. What data type is the "Vendor Name" field? What data type is the "Amount" field? What data type is the "Date" column? Answer below. **Answer** Vendor Name is a character type, Amount is a number column, and date is a character column. 
...
**Answer** at least 547. I think that this zipcode might've dropped the zero, we looked at it in class and it was downtown Boston. 
...
**Task** Run the following codeblock to count the number of payments. Looking at the result, what jumps out at you as potentially problematic? **Answer** N/As are there, L instead of loan, Contract is in there, miskeying something, could be to hide data. 
...
**Answer** two grants and/or loans that are under 100 dollars. The low amounts are really low, aka outliers that can potentially skew an average. 
...
**Task** Run the following codeblock to use the `clean_names()` function from janitor to standardize column names. How does the function change the headers? Answer below. **Answer** It can replace spaces with underscores and make the capitalization all lowercase to the headers.
...
**Task** Run the following codeblock to check for duplicate rows using get_dupes(). How many duplicates are possible? **Answer** Might be 50 duplicates. 
...
**Task** Run the following codeblock to use distinct() to get rid of duplicate rows. How many rows does the new dataframe have? Answer below. **Answer** About 10 rows now. 
...
**Task** Run the following codeblock to use str_sub() to convert the ZIP codes that have nine digits to five digits, creating a new column. Look at the difference in the result - what changed? **Answer** Trimmed zip5 to just 5 characters, made a new column. 
...
**Task** Write and run code to answer our original question: which zip code has gotten the most amount of money from the Maryland Tourism Board? 21202, which is Baltimore, Maryland, right near Federal Hill and Fells Point, and the Inner Harbor because that's where you'd want tourists, and it's where the $$ is. 
