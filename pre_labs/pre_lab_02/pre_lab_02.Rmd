...
filter(colname operator some_value)
== exact matches
> >= numbers and dates
< <= numbers and dates
!=
...
**Answer** If we have a lot of columns and only want to look at a few at a time.
...
**Answer** Either one has to be true, not both. (Or is an additive filter, and is a restrictive one)
...
**Answer:** Now the year is first, the month is second, and the day is third, also no confusion with which day something could be *as it should be*
...
**Answer** We've rounded all the dates in that month down to the first day of the month. 
...
**Answer** total votes equals all of those variables (cox, moore, etc.) the value of total votes is going to be all of the candidates, and the write ins. Now we can take the total of Moore's votes and divide it by itself (get a percentage)
...
**Answer** It made the highest percentages at the top and helped us see the correlation between the higher percentage and the largest population.
...
**Answer** Not necessarily because these are smaller counties that had lower percentages for Moore. However, knowing this makes sense to understand that in Western Maryland he can visit and then move the margins a little by being cordial with them.
...
**Answer** str_to_title, converts the string to the title case, and str_to_sequence, converts the string to the sentence case. I looked it up on tidyverse.org. 
...
**Answer** We are trying to create a new variable where we find the state and make the value of state "MD", True says if anything else, make it "N/A". 
...
**Answer** Yes because Maryland is a pretty rich state? It also makes it much smaller and more digestable so now all we have to see is the money sum and the state. 
...
1. Can I use mutate to find particular instructors? Or how would I go about finding certain instructor data (is it just not there)

2.  Could I use string detect to find which addresses contributed to the Wes Moore's percentage? In the counties?

3.  Can I find the connection between Maryland expenses in certain counties and what the voter trends were? 
