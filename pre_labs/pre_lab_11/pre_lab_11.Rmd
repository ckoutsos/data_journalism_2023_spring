...
**Answer** 2 columns: a date, news release/news item column (it's small)
...
**Answer** time it came out? link to press release (url) this will allow us to be able to scrape the table and search the database
(phrase, person mentioned, etc)
...
**Answer** about 100 of them, the first 16 are listing items
...
**Answer** list of all of the things inside the li tags (which ones we want vs the ones we don't want)
...
**Answer** takes the output of the text of our li tags and saves it as a dataframe called releases. however, the name/date everythingis in one column
...
li tags that have a class called li listing item
```{r}
...
```
drop_na(title): drop the NAs
string squish: remove any leading or trailing spaces
date = mdy(date): format the date correctly
