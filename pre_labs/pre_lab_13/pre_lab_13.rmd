...
**Answer** Data that Reveal used in their story: all the states + DC (who won in 2016), majority voted for, and small businesses who received a loan and the amount of a loan per employee
```{r}
...
**Answer** scatterplot w/ line of best fit (trend line) percent of votes for trump increased: a percent of small businesses receiving ppp loans goes up too (positive)
```{r}
...
### Task 4: Plot data
**Task** Run the following code. Describe the plot. What do you see?
**Answer** For the amount that voted for trump, ppe amount per employee goes up a little? relationship isn't quite as real, fit line isnt as diagonal
```{r}
...
**Answer** ran correlate function, showed correlation coefficient for both businesses receiving ppe pct: .52 (mildly strong positive relationship) and ppe amount of loan per employee: 0.22 (weak positive relationship)
...
**Answer** has correlation coefficient at the bottom, and the p value. The p value shows statistical significance (lower the p value, the better) very little chance that the relationship found is random
```{r}
...
**Task** Run the following code. Describe the model output. What do you see?
**Answer** Calculated the correlation coefficient, and the p value is much larger. 0.1189. 
```{r}
...
T test compares averages

### Task 8: Average data
**Task** Run the following code. Describe the table. What do you see?
**Answer** calculating averages for the amount per employee and avg % of businesses receiving loans in trump vs clinton: a couple hundred for mean ppp amount and 9% for mean businesses getting ppe
```{r}
reveal_data %>%
  group_by(vote_2016) %>%
  summarise(
...
**Answer** calculated avgs, and a t value (-1.2) and a p value (0.22) (high)
```{r}
...
### Task 10: Run t-test
**Task** Run the following code. Describe the output. What do you see?
**Answer** t value is -3.182 (stronger if it gets in either direction) pvalue is 0.002 (smaller): states that trump won small businesses in those states (higher % of businesses got loans in trump states rather than clinton states)
```{r}
