...
**Answer** data that we scraped last week plus a fourth column: text of each press release
...
**Answer** It gives the unique values but doesn't give enough uniqueness aka gives the same word twice saying that Dog and dog are both unique when they really aren't 
...
**Answer** made a new dataframe of one column called word with 647091 rows by giving us only a few words that are "unique"
...
**Answer** gives us top 25 words (the, and, to, of, in) all super common words. not interesting
...
**Answer** getting rid of a word that's in unique words and stop words. leaving us with more "unique" words.
...
**Answer** 2021: covid, health, 19, communities 2022: health, baltimore, funding 2023: women, rights, fire, etc 2023 is different than 2022 and 2021. we have single words that stand out to us w tiny percentages 
...
**Task** Run the following code and describe the results.
**Answer** for a two word phrase, we don't want either word in our stop words. after we get the bigram, they have to be broken apart again and test to see if the word is in our stop words (then filter it out) != exclude filter. everything left over is a bigram we want to include
...
**Task** Run the following code and describe the results.
**Answer** just 2022, covid 19 is still there but not as significant (project location funding, amount included, baltimore city)
...
**Task** Run the following code and describe the results.
**Answer** 2023: offshore wind, tax credit, equal rights, etc. different issues than before and focusing on different priorities. oral health???
...
**Answer** positive or negative connotation (context matters) for words. 
