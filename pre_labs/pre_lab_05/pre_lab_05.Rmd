...
**Task** Load the data without headers by running the codeblock below. What problems remain? **Answer** x1 x2 x3 aka placeholder names are the column headers now. We still have to have column names
...
**Task** Run the codeblock below. What else do we need to do? **Answer** We still need to get rid of the periods 

if you want a column with a year in it, make sure the column has something else in it
...
fentanyl_deaths
```
Slightly negative: St. Mary's became St Mary's (no period)

takeaways:
trends are going in wrong direction
growth is exponential in deaths
Carroll County exception to this, but general trend is that it's getting worse
