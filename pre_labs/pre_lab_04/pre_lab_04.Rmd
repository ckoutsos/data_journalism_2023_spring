...
**Task** Run the following code to group and count records in the Grantee column, and filter to examine only recipients that start with W. Name a problem that you see that is preventing proper grouping. **Answer** There are grantees that the same as others, so we see multiple copies of the same thing but written out differently (abbreviations, caps, spelling, etc.)
...
**Answer** Refinr takes two things, and sees where the overlap is for every grantee (so similar that they are going to be the same thing). Two changes the package made are that they made the Charles County Arts Alliance all caps, and Montgomery County Chamber Comm Foundation Inc into all caps.
...
**Task** Run the following code to group by grantee_clean and Grantee, then filter for recipients that start with W. Name three changes the refinr function made in the first 30 results. **Answer** It didn't do a lot for the first 30 results, there's really only one. One found was Washington Adventist, but it didn't clean up Commissione and commissioners. It didn't fix a lot of the data and makes conservative guesses. 
...
**Answer** No, it hasn't made substantial changes. 
