...
```{r}
 write_csv(media_expenses, "data/m2expenses.csv")
```
...
**A1.** ok so I went to openrefine and merged all the N/As in the same cluster. Is that bad? I also did the writecsv file above, so my code for that is above. I was honestly confused about how to write the code to isolate the media expenses where the name of the payee is not NA, so I skipped that step and went straight to the write csv file and openrefine.

Ok so the committee with the most spending is Friends of Jain Ashwani (I feel like that's definitely wrong)

```{r}
md_media_expenses <- payee_clean

md_media_expenses %>%
filter(payee_clean == "FACEBOOK" | payee_clean == "META") %>%
...
**A2.** They are mostly candidates or friends of the candidates. Issues they seem to care about are disposable income, decreased cost of living, public transit, taxes, education, and are mostly Democratic candidates for governor. (w/ exception of Billy Boniface and Amanda Fiedler). The ads are unfortunately not popping up for me on Facebook, they say "We took down this ad after it started running because the disclaimer doesn't follow our Advertising Standards." However, they were paid for by "Friends of Jain Ashwani"
...
**A3.** Wes Moore, Rushern L. Baker, Douglas F. Gansler, Ralph Jaffe, Tom Perez and Jerome Segal. Yes they did run ads on Facebook, according to the ad library. 
...
But also Moore Miller for Maryland Slate. I don't know if this is all of the advertising dollars but to try and do this I would maybe include another filter that filters Facebook?I dont know if I did that right but 

```{r}

md_media_expenses %>%
  filter(expense_purpose == "Online Advertising", payee_clean == "FACEBOOK"| payee_clean == "META") %>%
  group_by(committee_name) %>%
  summarise(total_amount = sum(amount)) %>%
  arrange(desc(total_amount))
...
**A5** We don't know a lot about which committee names stand for which people. I think that if I had clustered more in OpenRefine, I would have an easier time of finding this information. A story idea I could have would be how online advertising plays a role in winning elections. 
